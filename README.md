...
```rust
use dactyl-db::{self, Rows};

fn main() -> Result<(), dactyl-db::DactylError> {
    let rows: Rows = dactyl-db::read("select id, title, status from todos", true)?;
...
configuration (see [How dactyl-db picks the adapter](#how-dactyl-db-picks-the-adapter)).

## Public surface

- `dactyl-db::read(query: &str, optimize: bool) -> Result<Rows, DactylError>`
- `dactyl-db::write(query: &str, optimize: bool) -> Result<Rows, DactylError>`
- `dactyl-db::query!("...")` — compile-time-analyzed query macro
- `Rows`, `Row`, `DactylError` (result / error types)

Nothing else is exported at the crate root.

## How dactyl-db picks the adapter
...
For backwards compatibility, dactyl-db also supports legacy environment variables (`DACTYL_NEON_ENDPOINT`, `DACTYL_NEON_BEARER`, `DACTYL_SQLITE_PATH`, `DACTYL_SQLITE_ROOT`).
...
- DecapodLabs/dactyl-db#1 — bootstrap crate
